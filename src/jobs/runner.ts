...
import { searchKeyword } from "../tiktok/search.ts";
import {
  CaptchaError,
...
async function runDiscovery(id: string, keywords: string, target: number, filters: Filters): Promise<Job> {
  ...
  try {
    ...
    for (const term of terms) {
      const found = await withHumanRetry(id, page, () => searchKeyword(page, term, target));
    ...
    return updateJob(id, {
      status: "done",
      progress,
      result: { measured: progress.measured, keywords },
    });
  } catch (error) {
